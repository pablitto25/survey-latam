...
export default function Form() {
  ...
  const [formData, setFormData] = useState<FormData>({
    ...
    marcasCommercial: [], // Inicializar con un array vacío
  ...
  const handleCheckboxChange2 = (label: string) => {
    setSelectedMarcas((prevSelected) => {
      ...
      setFormData({ ...formData, marcasCommercial: newSelected });
  ...
  const handleCheckboxChange = () => {
    ...
    if (!otrosChecked) {
      ...
      setFormData({ ...formData, marcasCommercial: [...selectedMarcas, formData.otrasMarcas] });
    }
  };
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    try {
      const response = await fetch('https://api.latamly.com/api/commercialpartners/create', {
        method: 'POST',
        headers: {
  ...
  const validateStep = () => {
    switch (currentStep) {
      case 1:
        ...
        const isMarcasSelected = formData.marcasCommercial && formData.marcasCommercial.length > 0;
            ...
            {currentStep === 4 && (
              <div>
                ...
                <div className="pt-14 pl-8 pb-14 pr-8 bg-[#F3EFEF]">
                  <div className="flex flex-row">
                    <div>
                      <p>¿Quién considera un referente lider en el rubro / sector "Trade"?</p>
                    </div>
                  </div>
                  <div>
                    <Textarea
